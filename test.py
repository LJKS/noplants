...
for i in range(num_tests):
    ...
    plt.title('target')
    plt.subplot(222)
    plt.imshow(np.squeeze(out_img[:,:,0]), cmap='autumn')
    plt.title('carrot')
    plt.subplot(223)
    plt.imshow(np.squeeze(out_img[:,:,1]), cmap='summer')
    plt.title('bad plant')
    plt.subplot((224))
    plt.imshow(np.squeeze(out_img[:,:,2]), cmap='winter')
    plt.title('ground')
    plt.show()
    print(np.sum(out_img,-1))
